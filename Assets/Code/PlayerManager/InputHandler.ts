...
import PlayerManager from "./PlayerContainer";
import PlayerGUI from "Code/PlayerGUI/PlayerGUI";
import { Mouse } from "@Easy/Core/Shared/UserInput";
import AnimationManager from "Code/AnimationManager/AnimationManager";
...
export class InputHandler {
    ...
    public OnStartClient(): void {
        ...
        Airship.Inventory.ObserveLocalHeldItem((stack) => {
            ...
            if (!this.heldItemName) {
                AnimationManager.Get().PlayUnequipItem();
                return;
            }
            AnimationManager.Get().PlayEquipItem();
    ...
    public OnToolUse(toolComponent: ToolComponent): void {
        ...
        if (miningToolComponent) {
            ...
            switch (miningToolComponent.damageCategory) {
                case BlockDamageCategory.Axe:
                    animation = () => AnimationManager.Get().PlaySwingAxe();
                    break;
                case BlockDamageCategory.Pickaxe:
                    animation = () => AnimationManager.Get().PlaySwingPickaxe();
                    break;
                default:
                    break;
            }
        ...
        const tick = (() => {
            ...
            if (miningToolComponent) { animation(); this.OnMiningToolActivated(); }
